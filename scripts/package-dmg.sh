#!/usr/bin/env bash
# Build Parley and package it into a drag-to-install .dmg.
...
CONFIG="${1:-Release}"
DD="$ROOT/.build/dd"
APP="$DD/Build/Products/$CONFIG/Parley.app"
DIST="$ROOT/dist"
DMG="$DIST/Parley.dmg"
...
mkdir -p "$DIST"
STAGE="$(mktemp -d)"
cp -R "$APP" "$STAGE/Parley.app"
ln -s /Applications "$STAGE/Applications"

echo "▸ dmg"
rm -f "$DMG"
hdiutil create -volname "Parley" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
rm -rf "$STAGE"
