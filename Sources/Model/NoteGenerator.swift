...
actor NoteGenerator {
    ...
    Ты ассистент деловых встреч. По транскрипту (реплики с ролями «Вы» и «Собеседник») верни \
    СТРОГО JSON без пояснений и markdown:
    {"summary": ["короткие тезисы, 2-6 шт"], "decisions": ["принятые решения"], \
    "actions": [{"text": "конкретная задача в повелительном виде", "owner": "имя ответственного или null", "due": "срок если назван, напр. 'до пятницы' или null"}], \
    "topics": ["ключевые темы, 2-6 слов каждая"]}
    В actions включай: (1) ЯВНЫЕ поручения — фразы «создай/поставь задачу», «запиши задачу», \
    «надо/нужно сделать», «не забудь»; (2) назначения — «X сделает Y»; (3) договорённости с действием. \
    Формулируй кратко и по делу, извлекай ответственного и срок если названы.
